...
!    if(icollapse==1)call collapse(dtc,iflag_coll)
    ...
    curr_time=curr_time+dtc
      write(500,'(5es15.5)')curr_time*1.d15,energy/conv,sum(cdabs(ci)**2)
  enddo
  call write_output(1,1)
...
c00=631.182*conv
d00=2.75293*1.d10
x0_00=0.332921*1.d-10
y0_00=1.15263*1.d-10

a01=-276.12*conv
b01=0.637437*1.d10
c01=11.7655*conv
d01=9.24953*conv
x0_01=0.849374*1.d-10
y0_01=0.481028*1.d-10
...
d11=2.4752*1.d10
e11=233.77*conv
x0_11=1.41709*1.d-10
y0_11=1.29052*1.d-10
...
subroutine compute_potential(H_diab,delV_dels)
  ...
  ex_pot=a11*(1-exp(-b11*(x(1)-x0_11)))**2+c11*(1-exp(-d11*(x(2)-y0_11)))**2+e11
  gr_pot=a00*exp(-b00*(x(1)-x0_00))+c00*(1-exp(-d00*(x(2)-y0_00)))**2
  h1=ex_pot-gr_pot

  dh1(1)=2*a11*b11*dexp(-b11*(x(1)-x0_11))*(1-exp(-b11*(x(1)-x0_11)))+a00*b00*exp(-b00*(x(1)-x0_00))
dh1(2)=2*c11*d11*dexp(-d11*(x(2)-y0_11))*(1-exp(-d11*(x(2)-y0_11)))-2*c00*d00*dexp(-d00*(x(2)-y0_00))*(1-exp(-d00*(x(2)-y0_00)))


  H_diab(1,1)=h1
  delV_dels(1,1,:)=dh1(:)
  coup=a01*exp(-b01*(x(1)-x0_01))+c01*1.d10*(x(2)-y0_01)+d01

  !H_diab(1,2:nquant)=Vc
  !H_diab(2:nquant,1)=Vc
  do i=1,nquant/2
    Vc(nquant/2-i+1)=coup*sqrt(1.d0/2.d0*weights(i))
    Vc(nquant/2+i)=coup*sqrt(1.d0/2.d0*weights(i))

    der_pos(nquant/2-i+1,1,1)=-a01*b01*dexp(-b01*(x(1)-x0_01))*sqrt(1.d0/2.d0*weights(i))
    der_pos(1,nquant/2-i+1,1)=der_pos(nquant/2-i+1,1,1)

    der_pos(nquant/2+i,1,1)=-a01*b01*dexp(-b01*(x(1)-x0_01))*sqrt(1.d0/2.d0*weights(i))
    der_pos(1,nquant/2+i,1)=der_pos(nquant/2+i,1,1)

    der_pos(nquant/2-i+1,1,2)=c01*sqrt(1.d0/2.d0*weights(i))
    der_pos(1,nquant/2-i+1,2)=der_pos(nquant/2-i+1,1,2)

    der_pos(nquant/2+i,1,2)=c01*sqrt(1.d0/2.d0*weights(i))
    der_pos(1,nquant/2+i,2)=der_pos(nquant/2+i,1,2)
  enddo
  ...
  do i=2,nquant
   ...
   delV_dels(i,1,1)=der_pos(i-1,1,1)
   delV_dels(1,i,1)=der_pos(1,i-1,1)

   delV_dels(i,1,2)=der_pos(i-1,1,2)
   delV_dels(1,i,2)=der_pos(1,i-1,2)
...
!do i=1,nquant
!write(502,'(20es17.7)')delV_dels(i,:,1)
!write(503,'(20es17.7)')H_diab(i,:)/conv
!enddo
...
!  acc_cl=mass(1)*omg_B**2*x(1)
  ...
  pot_cl=a00*exp(-b00*(x(1)-x0_00))+c00*(1-exp(-d00*(x(2)-y0_00)))**2
  acc_cl(1)=-a00*b00*dexp(-b00*(x(1)-x0_00))
  acc_cl(2)=2*c00*d00*dexp(-d00*(x(2)-y0_00))*(1-dexp(-d00*(x(2)-y0_00)))

end subroutine potential_classical
!-----------------------------------------------------------------  
subroutine set_NO
implicit none
real*8::omega_NO,vib_energy,mass_N,mass_O,N(3),O(3),COM(3),rnd,red_mass,total_mass,planck_const,e_kin
real*8::l_x,l_y,box_len_x,box_len_y,f0,gamma0,rel_x,rel_v,theta,phi,r0_N_O,N_vel(3),O_vel(3),r_x,r_y
integer::vib_state
...
f0=638.5*conv
gamma0=2.743*10d10
r0_N_O=1.15077*1.d-10
planck_const=6.62607*1.d-34
...
call random_number(rnd)
COM(1)=l_x+box_len_x*rnd
call random_number(rnd)
COM(2)=l_y+box_len_y*rnd
COM(3)=11.d-10

omega_NO=dsqrt(2.d0*f0*gamma0**2/red_mass)
vib_energy=(0.5+real(vib_state))*planck_const*omega_NO/(2.d0*pi)

call random_number(rnd)
theta=2.d0*pi*rnd
rel_x=dsqrt(2.d0*vib_energy/(red_mass*omega_NO**2))*dcos(theta)
...
N_vel(3)=rel_v*dcos(theta)*0.5d0*mass_O/total_mass

O_vel(1)=-rel_v*dcos(phi)*dsin(theta)*0.5d0*mass_N/total_mass
O_vel(2)=-rel_v*dsin(phi)*dsin(theta)*0.5d0*mass_N/total_mass
O_vel(3)=-rel_v*dcos(theta)*0.5d0*mass_N/total_mass


x(2)=dsqrt(sum((N(:)-O(:))**2))
x(1)=N(3)

v(1)=N_vel(3)-dsqrt(2*e_kin/total_mass)
v(2)=rel_v
end subroutine set_NO
